...
class MutualSelfAttention(MutualAttentionBase):
    ...
    def attn_mask_cal(self, masks, num_heads, H, W, dtype):
        ...
        # 第一种：直接全1，但是会把前景信息引入到背景信息：
        if True:
            target_attention_mask = torch.zeros_like(attention_mask)
        # 第二种：用attention_mask(即：q和source的key，即inversion信息)，但是q维度中相应前景部分改为可见，即上下两个小方块:
        if False:
            target_attention_mask = attention_mask.clone()
            index_tuple = torch.where(attention_mask<0)
            target_attention_mask[index_tuple[0], index_tuple[2], index_tuple[1]] = 0
        if False:
            target_attention_mask = attention_mask.clone()
            target_attention_mask = rearrange(target_attention_mask, "f q_shape k_shape -> f k_shape q_shape")
            target_attention_mask = torch.where(target_attention_mask == 0, float('-inf'), 0)
    ...
    def attn_batch(self, q, k, v, masks, num_heads, attention_mask):
        ...
        # k_source_fg k_source_bg 的维度搞错了：
...
class AttentionControlEdit(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str):
        ...
        if is_cross or (self.num_self_replace[0] <= self.cur_step < self.num_self_replace[1]):
            ...
            attn_repalce = cond_edit
            attn_base = cond_invert.unsqueeze(0)
            if is_cross:
                alpha_words = self.cross_replace_alpha[self.cur_step]
                attn_replace_new = self.replace_cross_attention(attn_base, attn_repalce) * alpha_words + (1 - alpha_words) * attn_repalce
                attn = torch.cat([attn_replace_new, attn_base], dim=0)
            else:
                attn_replace_new = self.replace_self_attention(attn_base, attn_repalce, place_in_unet)
                attn = torch.cat([attn_replace_new, attn_base], dim=0)
            # attn = attn.reshape(self.batch_size * h, *attn.shape[2:])
        return attn
    
    def __init__(self, prompts, tokenizer, device, num_steps: int,
...
class AttentionReplace(AttentionControlEdit):
    ...
    def __init__(self, prompts, tokenizer, device, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
...
class AttentionReweight(AttentionControlEdit):
    ...
    def __init__(self, prompts, tokenizer, device, num_steps: int, cross_replace_steps: float, self_replace_steps: float, equalizer,
...
def make_controller(config, tokenizer, device, prompts: List[str], is_replace_controller: bool, cross_replace_steps: Dict[str, float], self_replace_steps: float, blend_words=None, equilizer_params=None) -> AttentionControlEdit:
    if blend_words is None:
        lb = None
    else:
        lb = LocalBlend(prompts, blend_words, tokenizer, device, config)
    if is_replace_controller:
        controller = AttentionReplace(prompts, tokenizer, device, config.num_inference_step, cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps, local_blend=lb)
    else:
        controller = AttentionRefine(prompts, tokenizer, device, config.num_inference_step, cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps, local_blend=lb)
    if equilizer_params is not None:
        eq = get_equalizer(tokenizer, prompts[1], equilizer_params["words"], equilizer_params["values"])
        controller = AttentionReweight(prompts, tokenizer, device, config.num_inference_step, cross_replace_steps=cross_replace_steps,
                                       self_replace_steps=self_replace_steps, equalizer=eq, local_blend=lb, controller=controller)
    return controller


# 用p2p的方式修改cross attn，包括replace，refine，reweight三种：
def regiter_crossattn_editor_diffusers_p2p(model, controller): 
    ...
    def ca_forward(self, place_in_unet):
        def forward(hidden_states, encoder_hidden_states=None, attention_mask=None): # video_length=None, normal_infer=False, source_masks=None, target_masks=None, rectangle_source_masks=None
                ...
                ########################################################################
                # 计算 softmax, 然后用 controller 计算注意力map:
